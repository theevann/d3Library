...
(function () {
    ...
    var sunburst = function (args){
        ...
        //Optional arguments
        that.normalSunburst = !(args.bilevel || false); // Have a normal or bilevel sunBurst
        ...
        that.proportion = Math.min(that.width, that.height) / 550;
        that.fontSize = 16 * that.proportion;
        if(that.width === 0 || that.height === 0){
            alert("Please give width / height to the container");
            throw "No dimension set to container";
        }
        
        heightDivBC *= that.proportion;
        for (var a in b) { b[a] *= that.proportion;}
        ...
        exp.append("span").attr("id","percentage")
            ...
            .style("font-size", that.fontSize * 2.5 + "px")
            .text(" ");
        exp.append("br");
        exp.append("span")
            .attr("id","percentageText")
            .style("font-size", that.fontSize * 1.5 + "px")
            .style("font-family","sans-serif")
            .style("hyphens","auto")
            .style("word-wrap","break-word")
            .text(that.explanationText);
        ...
        exp.style("visibility", "hidden").style("position","absolute").style("width", 0.8 * that.radius + "px").style("text-align","center").style("top",(0.5 * (that.height-heightDivBC) - parseFloat(exp.style("height")) / 2) + "px").style("left",(0.5 * that.width - parseFloat(exp.style("width")) / 2) + "px").style("color","#666");
    ...
    sunburst.prototype.initializeBreadcrumbTrail = function() {
        // Add the svg area.
        var trail = this.container.select("#sequence").attr("font-size", this.fontSize + 'px').append("svg:svg")
            .attr("width", "100%")
            .attr("height", "100%")
            .attr("id", "trail");
  ...
  // Generate a string that describes the points of a breadcrumb polygon.
    sunburst.prototype.breadcrumbPoints = function(d, i) {
        ...
        var w = ctx.measureText(d.name).width + 20 * this.proportion;
    ...
    sunburst.prototype.updateBreadcrumbs = function(nodeArray, percentageString) {
        ...
        var canvas = document.createElement('canvas');
        var ctx = canvas.getContext("2d");
        ctx.font = that.fontSize + "px 'sans-serif'";

        // Data join; key function combines name and depth (= position in sequence).
        var g = that.container.select("#trail")
            .selectAll("g")
        ...
        entering.append("svg:polygon")
            .attr("points", that.breadcrumbPoints.bind(that))
            .style("fill", function(d) { return d.fill; });

        entering.append("svg:text")
            .attr("x", function(d){ var w = ctx.measureText(d.name).width + 20 * that.proportion; return(w + b.t) / 2})
            .attr("y", b.h / 2)
            .attr("dy", "0.35em")
            .style("font-family","sans-serif")
        ...
        that.container.selectAll("#sequence text").style("font-weight","600").style("fill","#fff");

        // Set position for entering and updating nodes.
        g.attr("transform", function(d) {
            var str = "translate(" + translate + ", 0)"
            translate += ctx.measureText(d.name).width + 20 * that.proportion + b.t;
            return str;
        });
        ...
        that.container.select("#trail").select("#endlabel")
            .attr("x", translate+30 * that.proportion)
            .attr("y", b.h / 2)
            .attr("dy", "0.35em")
