...
(function(){
    ...
    var chart = function (args){
        ...
        yExt = [d3.min(that.series,function(d){return d3.min(d.data,function(d){return d[1]});}),
                d3.max(that.series,function(d){return d3.max(d.data,function(d){return d[1]});})];
        xExt = [d3.min(that.series,function(d){return d3.min(d.data,function(d){return d[0]});}),
                d3.max(that.series,function(d){return d3.max(d.data,function(d){return d[0]});})];
        ...
        that.fill = that.series.map(function(d,i){return (typeof d.fill == "undefined") ? args.fill || false : d.fill;});
        that.sort = that.series.map(function(d,i){return (typeof d.sort == "undefined") ? args.sort || false : d.sort;});
        that.color = that.series.map(function(d,i){return d.color || args.color || colors(i%10);});
        that.fillColor = that.series.map(function(d,i){return d.fillColor || args.fillColor || that.color[i];});
        that.strokeWidth = that.series.map(function(d,i){return d.strokeWidth || args.strokeWidth || 2;});
        that.interpolate = that.series.map(function(d,i){return d.interpolate || args.interpolate || "linear";});
    ...
    chart.prototype.initGuideLine = function() {
        ...
        var mousemove = function() {
            ...
            var index = that.series.map(function(d){return bisect(d.data, x0,1);});
            var d0 = that.series.map(function(d,i){return d.data[index[i] - 1];});
            var d1 = that.series.map(function(d,i){return d.data[index[i]];});
            var dd = that.series.map(function(d,i){return d1[i] ? (x0 - d0[i][0] > d1[i][0] - x0 ? d1[i] : d0[i]) : d0[i];});
            var serieFollowed = that.serieFollowed != -1 ? that.serieFollowed : min(dd.map(function(d,i){return Math.abs(x0 - dd[i][0]);}));
            ...
            that.helpGroup.selectAll("text")
                .attr("x", function(d,i){return that.x(dd[i][0]);})
                .attr("y", function(d,i){return that.y(dd[i][1]);})
                .attr("dx", 10)
                .text(function(d,i){return d3.format(",.2f")(dd[i][1]);});
        };
